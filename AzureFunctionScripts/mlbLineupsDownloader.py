...
url = f"https://www.mlb.com/starting-lineups/2022-04-09"
...
for item in items:
    ...
    for i in range(0, len(awayLineup)):
        print(awayLineup[i]['href'].split('-')[-1] + "   " + awayLineup[i].text.strip() + "   " +  awayPositions[i].text.strip())
    ...
    for i in range(0, len(homeLineup)):
        print(homeLineup[i]['href'].split('-')[-1] + "   " + homeLineup[i].text.strip() + "   " +  homePositions[i].text.strip())
